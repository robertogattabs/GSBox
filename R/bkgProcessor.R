...
#' @export
#'
bkgProcessor <- function( input_folder.dir = NA , output_folder.dir = NA ,  tmp_folder.dir=NA,
                       RscriptPathName = NA,
  ...
  start <- function() {
    ...
    while( get.current.state() =="RUNNING" ) {
      ...
      for( run.id in names(res$lst.run)) {
        ...
        executionFolderFullName <- paste(c(global.lst.tmp_folder$path,"/tIUID_",current.tokenInstanceUID,"/",executionFolderName),collapse = '')
  ...
  cleanUp.tmp.folders <- function(run = TRUE, temp = TRUE , current.tokenInstanceUID = NA) {
    if(run==TRUE ) {
